...
const MessageInput = ({ className, selectedUser }) => {
        ...
        reader.onloadend = async () => {
            if (file.type.startsWith("image/")) {
                ...
                try {
                    ...
                    setIsSending(true);
                    await sendMessage(formData, { type: "image", mediaUrl: reader.result });
                    setImagePreview(null);
                } catch (err) {
                ...
                try {
                    ...
                    setIsSending(true);
                    await sendMessage(formData, { type: "video", mediaUrl: reader.result });
                    setVideoPreview(null);
                } catch (err) {
    ...
    const handleSendMessage = async (e, payload = {}) => {
        ...
        if (!msgText && !imgUrl && !vidUrl && !stickerUrl && !gifUrl && !text.trim()) return;
        if (!selectedUser?._id) {
        ...
        try {
            if (gifUrl) {
                ...
                await sendMessage(formData, { type: "gif", gif: gifUrl });
            } else if (stickerUrl) {
                const formData = new FormData();
                formData.append('sticker', stickerUrl);
                await sendMessage(formData, { type: "sticker", sticker: stickerUrl });
            } else if (imgUrl) {
                const formData = new FormData();
                formData.append('image', imgUrl);
                await sendMessage(formData, { type: "image", mediaUrl: imgUrl });
            } else if (vidUrl) {
                const formData = new FormData();
                formData.append('video', vidUrl);
                await sendMessage(formData, { type: "video", mediaUrl: vidUrl });
            } else if (text.trim()) {
                const formData = new FormData();
                formData.append('text', text.trim());
                await sendMessage(formData, { type: "text" });
            }
            setText("");
            setImagePreview(null);
            setVideoPreview(null);
